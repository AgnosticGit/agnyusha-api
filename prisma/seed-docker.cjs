/**
 * Production seed runner for the Docker API image (uses compiled dist helpers).
 * Usage (inside container): node prisma/seed-docker.cjs
 */
const fs = require('fs');
...
const { legacyBadgeFields } = require('../dist/src/products/badge.util');
const { parseLabelTxt } = require('../dist/src/products/label-txt.parser');
const {
  normalizeProductSections,
} = require('../dist/src/products/product-sections.util');
const { slugify } = require('../dist/src/products/product.util');
...
function copySeedImage(srcName, destName) {
