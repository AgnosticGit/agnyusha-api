...
6. Один запрос. Записываются трек, код и текст статуса перевозчика. Для СДЭК и Яндекса также поднимается статус заказа на сайте (`Передан в доставку` / `В пути` / `Можно забирать` / `Получен`) — только вперёд. Почта России: пока только поля трекинга (Tracking API — следующий шаг).
