...
6. Один запрос. Записываются трек, код и текст статуса перевозчика. Для СДЭК и Яндекса также поднимается статус заказа на сайте — только вперёд и только после реальной сдачи груза: `CREATED`/`ACCEPTED` оставляют «Собирается» (`PAID`); приём на складе → «Передан в доставку»; транзит → «В пути»; ПВЗ → «Можно забирать»; вручение → «Получен». Почта России: пока только поля трекинга (Tracking API — следующий шаг).
